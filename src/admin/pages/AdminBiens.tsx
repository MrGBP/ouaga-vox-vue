...
import { Search, Check, X, Pencil, Zap, ExternalLink } from 'lucide-react';
...
import type { AdminPropertyStatus } from '@/admin/types';
...
export default function AdminBiens() {
  ...
  const filtered = useMemo(() => {
    return adminProperties.filter(p => {
  ...
  }, [tab, quartierFilter, typeFilter, search]);

  const paged = filtered.slice(page * 25, (page + 1) * 25);
  const totalPages = Math.ceil(filtered.length / 25);
  const selected = selectedId ? adminProperties.find(p => p.id === selectedId) : null;

  return (
    <div className="relative">
      <AdminPageHeader title="Gestion des biens" subtitle={`${filtered.length} biens`} />
      ...
      <div className="flex gap-1 mb-4 overflow-x-auto">
        {TABS.map(t => {
          const count = t.filter === 'all' ? adminProperties.length : adminProperties.filter(p => p.adminStatus === t.filter).length;
          return (
            <button
              key={t.filter}
              onClick={() => { setTab(t.filter); setPage(0); }}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium whitespace-nowrap transition-colors ${tab === t.filter ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground hover:bg-muted'}`}
            >
              {t.label} ({count})
            </button>
          );
              ...
              <tr key={p.id} onClick={() => setSelectedId(p.id)} className="border-b border-border hover:bg-muted/50 cursor-pointer transition-colors">
                <td className="px-4 py-2.5">
                  {p.images?.[0] ? (
                    ...
                    <div className="w-12 h-12 rounded-lg bg-blue-50" />
                  )}
                </td>
                <td className="px-4 py-2.5">
                ...
                <td className="px-4 py-2.5">
                  <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold text-white" style={{ background: TYPE_COLORS[p.type] || '#64748b' }}>
                    {getTypeLabel(p.type)}
                  </span>
                </td>
                <td className="px-4 py-2.5 text-xs font-semibold text-foreground whitespace-nowrap">
                  {isTypeFurnished(p.type)
                    ? `${pricePerNight(p.price).toLocaleString('fr-FR')} FCFA/nuit`
                ...
                <td className="px-4 py-2.5">
                  <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                    {(p.adminStatus === 'pending' || p.adminStatus === 'reviewing') && (
                      <>
                        <button className="w-8 h-8 rounded-md flex items-center justify-center bg-emerald-100 text-emerald-700 hover:bg-emerald-200"><Check size={14} /></button>
                        <button className="w-8 h-8 rounded-md flex items-center justify-center bg-red-100 text-red-700 hover:bg-red-200"><X size={14} /></button>
                      </>
                    )}
                    <button className="w-8 h-8 rounded-md flex items-center justify-center bg-slate-100 text-slate-600 hover:bg-slate-200"><Pencil size={14} /></button>
                    {p.adminStatus === 'published' && (
                      <button className="w-8 h-8 rounded-md flex items-center justify-center bg-amber-100 text-amber-700 hover:bg-amber-200"><Zap size={14} /></button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-4">
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i} onClick={() => setPage(i)} className={`w-8 h-8 rounded-lg text-xs font-semibold ${page === i ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground hover:bg-muted'}`}>
              {i + 1}
            </button>
          ))}
        </div>
      ...
      {selected && (
        <div className="fixed top-0 right-0 bottom-0 w-[420px] bg-card shadow-2xl z-[200] overflow-y-auto border-l border-border">
          <div className="sticky top-0 bg-card z-10 flex items-center justify-between px-5 py-3 border-b border-border">
            ...
            <button onClick={() => setSelectedId(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-muted">
              <X size={16} />
            </button>
          </div>
          {selected.images?.[0] && <img src={selected.images[0]} alt="" className="w-full h-48 object-cover" />}
          <div className="p-5 space-y-4">
            <div className="flex items-center gap-2">
              <AdminBadge variant={selected.adminStatus} />
              <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold text-white" style={{ background: TYPE_COLORS[selected.type] || '#64748b' }}>
                {getTypeLabel(selected.type)}
              </span>
            </div>
            <div>
              <p className="text-lg font-bold text-foreground">
                {isTypeFurnished(selected.type) ? `${pricePerNight(selected.price).toLocaleString('fr-FR')} FCFA/nuit` : `${selected.price.toLocaleString('fr-FR')} FCFA/mois`}
              </p>
            ...
            <div>
              <p className="text-xs font-semibold text-foreground mb-1">Score qualité IA</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${selected.qualityScore}%`, background: selected.qualityScore < 40 ? '#dc2626' : selected.qualityScore < 70 ? '#d97706' : '#059669' }} />
                </div>
                <span className="text-sm font-bold">{selected.qualityScore}/100</span>
              </div>
            </div>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1">Statistiques</p>
            ...
            <div className="flex gap-2 pt-2">
              <button className="flex-1 h-9 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700">✓ Publier</button>
              <button className="flex-1 h-9 rounded-lg bg-red-600 text-white text-xs font-semibold hover:bg-red-700">✗ Rejeter</button>
              <button className="flex-1 h-9 rounded-lg bg-amber-500 text-white text-xs font-semibold hover:bg-amber-600">🚀 Boost</button>
            </div>
          </div>
        </div>
      )}
